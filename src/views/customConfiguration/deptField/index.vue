<template>
  ...
  import {
    addObj,
    delObj,
    getAssetsFieldByPage,
    putObj,
  } from "@/api/customConfiguration/deptField/index";
  import { tableOption } from "@/const/crud/customConfiguration/deptField/index";
  ...
  export default {
    ...
    methods: {
      ...
      selectionChange(list){
        this.ids = list.map(item => (item.typeId))
      },
  
      deleteBtn(row) {
        const ids = row ? [row.typeId] : this.ids
        if(!ids.length) {
